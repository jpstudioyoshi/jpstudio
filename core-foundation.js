...
async function claudeAPI(opts) {
  ...
  // Track usage — always log and track, even if opts.track was omitted.
  // Calls showing feature="unknown" come from local wrappers that bypass claudeAPI opts.
  const _feature = opts.track || 'unknown';
  console.warn(`[API] Claude call · feature="${_feature}" · in=${data.usage?.input_tokens ?? '?'} out=${data.usage?.output_tokens ?? '?'} tokens`);
  if (data.usage && typeof apiUsageTrack === 'function') {
    apiUsageTrack(_feature, data.usage.input_tokens || 0, data.usage.output_tokens || 0);
  }
...
const STORAGE_BACKUP_KEYS = [
  ...
  'LESSON_NOTES', 'GRAMMAR_POINTS', 'GRAM_SENT_HISTORY',
...
function setButtonGroupActive(group, activeId, activeColor = 'var(--teal)') {
  const btns = document.querySelectorAll(`[data-btn-group="${group}"]`);
  btns.forEach(btn => {
