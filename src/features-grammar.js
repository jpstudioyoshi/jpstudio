...
function gramSentPopulateWeakPoints() {
  ...
  try {
    ...
    container.innerHTML = patterns.map(p =>
      `<button class="btn-ghost" style="font-size:0.75rem;padding:4px 8px" onclick="gramSentSelectWeakPoint(this,'${p.replace(/'/g,"\'")}')">` +
      `${p}</button>`
    ).join('');
...
async function particleDrillStart() {
  ...
  try {
    ...
    if (area) area.innerHTML = '<div style="font-family:var(--ui);font-size:0.82rem;color:#e05050;padding:12px">Generation failed: ' + e.message + '<br><button class="btn-ghost" style="margin-top:8px;font-size:0.78rem" onclick="particleDrillStart()">Try again</button></div>';
    return;
  }
  _pdLoading = false;
  particleDrillRender();
...
function showConjLookupG() {
  ...
  const verbTableHTML = verbRows.map(r =>
    `<tr style="${r.hi?'background:rgba(48,213,200,0.14)':''}">
      ...
      <td style="padding:5px 6px;border-bottom:1px solid rgba(255,255,255,0.06)"><button class="btn-ghost" style="font-size:0.72rem;padding:2px 5px" onclick="jpSpeak('${r.answer}')">🔊</button></td>
    </tr>`
  ).join('');
