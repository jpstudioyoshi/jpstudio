    ...
    return `<div class="it-issue" id="it-issue-${issue.id}" style="border:1px solid var(--border);border-radius:6px;margin-bottom:10px;overflow:hidden">
      ...
      <div id="it-expand-${issue.id}" style="display:none;padding:12px;border-top:1px solid var(--border)">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          ...
          <button class="btn-action btn-sm" onclick="itCopyPrompt(${issue.id})">Copy prompt</button>
          <button class="btn-action btn-sm" onclick="itEditResolution(${issue.id})">✏ Resolution</button>
          <button class="btn-action btn-sm" style="color:var(--red);border-color:var(--red)" onclick="itDelete(${issue.id})">Delete</button>
        </div>
      </div>
    </div>`;
  }).join('');
