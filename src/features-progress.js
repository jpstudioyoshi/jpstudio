...
// ══════════════════════════════════════════════════════════════════
// N5 GRAMMAR DEPENDENCY GRAPH
// ══════════════════════════════════════════════════════════════════
// Static graph. Nodes keyed by id. Used at briefing time for:
//   (a) prerequisite check — does an error's root lie upstream?
//   (b) unlock signal — which nodes are structurally ready to learn?

const N5_GRAPH = {
  nodes: [
    // Layer 0 — Absolute foundations
  ...
  // Map error taxonomy types to graph node ids
  // Used for prerequisite check: when an error of this type appears,
  ...
  // Mastery signals from the app mapped to node ids
  // Used for unlock check: when a mastery signal is strong, which nodes
  // can be considered prerequisite-satisfied?
  masteryNodeMap: {
    // CM counter mastery implies verb groups (you've drilled inflection)
    "counter_mastered":  ["verb-groups"],
    // Conjugation drill with form coverage implies these nodes
...
// ── Grammar graph signal helpers ──────────────────────────────────────────

// Signal A: prerequisite check
// Given recent error patterns, find nodes implicated and check if
// their prerequisites also show weakness — suggesting the root is upstream
function agentGrammarRootSignal(writingErrors, grammarErrors) {
  const signals = [];

  // Collect implicated node ids from writing errors
  ...
  // Also from grammar errors (form-based)
  ...
  // For each implicated node, check if any prerequisite is also implicated
  ...
  return signals.slice(0, 1); // surface at most one root signal
}

// Signal B: unlock readiness
// Walk the graph and find nodes where all prerequisites are satisfied
// by current mastery data but the node hasn't been explicitly drilled
function agentGrammarUnlockSignal(grammarErrors, conjSessionData) {
  const signals = [];

  // Build a set of "satisfied" node ids from available mastery data
  const satisfied = new Set();

  // Foundations are always satisfied
  ['copula','word-order','topic-wa','verb-concept','i-adj','and-to','possess-no',
   'ka','ne','yo','soshite'].forEach(id => satisfied.add(id));

  // Infer from grammar error data — if a form has been drilled (even with errors),
  // the underlying node has been encountered
  ...
  // Any recent conj session counts as verb-groups exposure
  if (conjSessionData)               { satisfied.add('verb-groups'); satisfied.add('masu-form'); satisfied.add('dict-form'); }

  // Find nodes that are NOT yet in satisfied but whose prerequisites ALL are
  const candidates = N5_GRAPH.nodes.filter(n => {
    if (satisfied.has(n.id)) return false;
    if (n.requires.length === 0) return false; // foundations already counted
    return n.requires.every(r => satisfied.has(r));
  });

  if (candidates.length > 0) {
    // Pick the two most structurally interesting (prefer te-spine)
...
// DRILL RECENCY TRACKING + GRAPHIC
// ══════════════════════════════════════════════════════════════════
...
function particleBreakdownRender() {
  ...
  for (const err of sorted) {
    ...
    html +=
      ...
      + '<button class="btn-ghost" style="font-size:0.65rem;padding:1px 5px;flex-shrink:0" onclick="errorDenylistAdd(\'' + safePattern + '\')" title="Ignore">\u229b</button>'
      + '</div>';
  }
  html += '</div>';
